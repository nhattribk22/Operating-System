...
{                                                   // no guarantee all given pages are mapped
  ...
  {
    // Get the page table entry for the current page number
    uint32_t *current_pte = &(caller->mm->pgd[pgn + pgit]);
    // Get the frame page number
    int current_fpn = fpit->fpn;
    // Set the frame page number in the page table entry
    *current_pte = current_fpn;
  ...
  // Move to the next page and frame
  pgit++;
  fpit = fpit->fp_next;
